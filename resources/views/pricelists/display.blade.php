<x-layouts.app title="Pricelist view">
    <div class="p-3">
        ...
        <div class="d-flex flex-column align-items-center mb-3">
            ...
            @if ($pricelist->phone or $pricelist->whatsapp)
                <div class="row mb-2">
                    @if ($pricelist->phone)
                        <div class="mb-1 d-flex align-items-center justify-content-center col">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                    ...
                    @if ($pricelist->whatsapp)
                        <div class="mb-1 d-flex align-items-center justify-content-center col">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                class="bi bi-whatsapp me-1" viewBox="0 0 16 16" style="color: forestgreen">
                                <path
            ...
            @if ($pricelist->facebook or $pricelist->instagram or $pricelist->twitter)
                <div class="row">
                    @if ($pricelist->facebook)
                        <a href="{{ $pricelist->facebook }}" class="col">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                class="bi bi-facebook" viewBox="0 0 16 16" style="color: blue">
                                <path
                    ...
                    @if ($pricelist->instagram)
                        <a href="{{ $pricelist->instagram }}" class="col">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                class="bi bi-instagram" viewBox="0 0 16 16" style="color: #E1306C">
                                <path
                    ...
                    @if ($pricelist->twitter)
                        <a href="{{ $pricelist->twitter }}" class="col">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                class="bi bi-twitter" viewBox="0 0 16 16" style="color: #00acee">
                                <path
        ...
        <x-ui.scroll-nav :categories="$pricelist->categories->where('parent_id', null)" />

        @foreach ($pricelist->categories->where('parent_id', null) as $category)
            <div class="container-fluid">
                <h1 class="mb-4 text-primary" id="p{{ $category->id }}">{{ $category->name }}</h1>
                <div class="row">
                ...
                @if ($category->subcategories)
                    @foreach ($category->subcategories as $subcategory)
                        <h3 class="text-secondary mb-4">{{ $subcategory->name }}</h3>
                        <div class="row">
